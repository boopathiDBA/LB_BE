module "search_offer_ext" {
  source  = "terraform-aws-modules/lambda/aws"
  version = "4.7.1"

  function_name = "${var.environment}-be-search-offer-ext-service"
  description   = "Searches OpenSearch and Google for matching offers"
  handler       = "app.delivery.rest.offer.ext.handler"
  runtime       = "python3.9"
  publish       = true
  timeout       = 60
  memory_size   = 512
  hash_extra    = "middleware-search-offer-ext"
